            ...
            <div class="md:w-2/3">
                ...
                <dl class="grid grid-cols-1 md:grid-cols-2 gap-2 mb-4">
                    <div>
                        ...
                        <dd class="text-white">{{ $alumni->nama_lengkap }}</dd>
                    </div>
                    <div>
                        <dt class="text-sm text-gray-400">NIM</dt>
                        <dd class="text-white">{{ $alumni->nim }}</dd>
                    </div>
                    <div>
                        <dt class="text-sm text-gray-400">Jenis Kelamin</dt>
                        <dd class="text-white">{{ ucfirst($alumni->jenis_kelamin) }}</dd>
                    </div>
                    <div>
                        <dt class="text-sm text-gray-400">Tahun Lulus</dt>
                        <dd class="text-white">{{ $alumni->tahun_lulus }}</dd>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <dd class="text-white">{{ $alumni->number_phone }}</dd>
                    </div>
                    <div class="md:col-span-2">
                        <dt class="text-sm text-gray-400">Alamat</dt>
                        <dd class="text-white">{{ $alumni->alamat }}</dd>
                    </div>
                </dl>
            </div>
        </div>
        ...
        @if ($alumni->status )
            <div class="bg-slate-800 p-4 rounded-lg">
                <div class="flex justify-between">
                    <div>
                        @if ($alumni->status->isBekerja())
                        <h5 class="font-medium text-white">Bekerja di: {{ $alumni->status->nama }}</h5>
                            <p class="text-primary-400">Jabatan: {{ $alumni->status->jabatan }}</p>
                            <p class="text-sm text-gray-400">Sejak {{ $alumni->status->tahun_mulai }}</p>
                        @else
                        <h5 class="font-medium text-white">Kuliah di: {{ $alumni->status->nama }}</h5>
                            <p class="text-primary-400">Jenjang: {{ $alumni->status->jenjang }}</p>
                            <p class="text-sm text-gray-400">Sejak {{ $alumni->status->tahun_mulai }}</p>
                        @endif
                    </div>
                    <div class="text-right">
                        @if ($alumni->status->isBekerja() && $alumni->status->gaji)
